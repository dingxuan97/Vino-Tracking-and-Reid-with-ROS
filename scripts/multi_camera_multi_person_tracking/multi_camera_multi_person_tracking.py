...
def run(params, config, capture, detector, reid):
	...
	tracker = MultiCameraTracker(capture.get_num_sources(), reid, config['sct_config'], **config['mct_config'], visual_analyze=config['analyzer'])
	...
	#	while thread_body.process:
		try:
			if not params.no_show:
				...
				if key == ord("s"):
					index = input("Enter the Index you want to track")
					target = True
					target_idx.append(index)
				if key == ord("d"):
					index = input("Enter the Index that you want to remove")
					target_idx.remove(index)
				if key == 27:
					break
			start = time.time()
			try:
...
def main():
	...
	parser.add_argument('-m', '--m_detector', type=str, required=False, default='/opt/intel/openvino/deployment_tools/open_model_zoo/tools/downloader/intel/person-detection-retail-0013/FP32/person-detection-retail-0013.xml',
	...
	parser.add_argument('--m_reid', type=str, required=False, default='/opt/intel/openvino/deployment_tools/open_model_zoo/tools/downloader/intel/person-reidentification-retail-0031/FP32/person-reidentification-retail-0031.xml',
