...
def visualize_multicam_detections(target_idx, frames, all_objects, fps='', show_all_detections=True,
    ...
    for i, (frame, objects) in enumerate(zip(frames, all_objects)):
        draw_detections(target_idx, frame, objects, show_all_detections)
        if vis is not None:
            if stack_frames == 'vertical':
